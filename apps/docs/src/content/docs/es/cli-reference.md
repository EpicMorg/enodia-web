...
description: Todos los comandos de enodia, tal como los describe --help.
---

Todos los comandos aceptan también `-h`/`--help`. Esta página se genera
a partir de esa misma salida y se mantiene sincronizada a mano con el
binario compilado real, en lugar de escribirse de memoria.
...
Tres subcomandos, sin opciones propias aparte de las globales.

### `enodia config path`

Muestra qué archivo de configuración se usaría realmente, según el orden
de búsqueda actual; consulte
[Configuración](/es/configuration/#ubicación-de-los-archivos).
...
Valida el archivo de configuración y sus referencias a credenciales,
incluido si el nombre `credentials:` declarado por un destino se resuelve
realmente en una entrada que la sonda entiende, todo ello sin conexión.

## `enodia products`

Enumera todos los productos compatibles; consulte
[Productos compatibles](/es/products/) para ver la tabla completa.
...
Muestra la versión de enodia, el commit y la fecha de compilación.

## `enodia about`

Muestra el logotipo de enodia y la información de compilación: versión,
commit, fecha de compilación, versión de Go, plataforma, número de
productos (sondas), licencia y URL del repositorio.

## `enodia completion`

Genera un script de autocompletado para el shell. Ejecute
`enodia completion --help` para el shell concreto que utilice (bash,
zsh, fish, powershell).
