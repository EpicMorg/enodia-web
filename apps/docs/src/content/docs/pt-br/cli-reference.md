...
description: Todos os comandos do enodia, direto do --help.
---

Todos os comandos também aceitam `-h`/`--help`. Esta página é gerada a
partir dessa mesma saída, mantida em sincronia manualmente com o binário
realmente compilado, e não escrita de memória.
...
Três subcomandos, sem flags próprias além das globais.

### `enodia config path`

Mostra qual arquivo de configuração seria de fato usado, dada a ordem de
busca atual — consulte [Configuração](/pt-br/configuration/#localização-dos-arquivos).
...
Valida o arquivo de configuração e as suas referências a credenciais —
inclusive se o nome em `credentials:` declarado em um alvo de fato resolve
para uma entrada que a sonda entende —, totalmente offline.

## `enodia products`

Lista todos os produtos suportados — consulte
[Produtos suportados](/pt-br/products/) para a tabela completa.
...
Mostra a versão do enodia, o commit e a data do build.

## `enodia about`

Mostra o logotipo do enodia e as informações do build: versão, commit, data
do build, versão do Go, plataforma, número de produtos (sondas), licença e
URL do repositório.

## `enodia completion`

Gera um script de autocompletar para o shell. Execute
`enodia completion --help` para o shell específico que você usa (bash, zsh,
fish, powershell).
