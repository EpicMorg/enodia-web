...
твоих сервисов), опциональный отдельный **`credentials.yaml`** и
...
`address` пишется ровно так, как ты бы его набрал — каждая проба
...
либо запусти `enodia config resolve`, чтобы увидеть, какую схему выберет
каждый таргет, не отправляя ни одного credential.
...
targets:
  - id: in-house-api
    ...
    parser:
      ...
      regex: 'v(\d+\.\d+\.\d+)'
      cleanregex: '^v'      # побеждает первая захватывающая группа; обрати внимание на написание строчными
      line: 1                # только для plaintext - какую строку читать
```

:::caution[Написание поля: `cleanregex`, а не `cleanRegex`]
У полей спецификации парсера нет явного маппинга в исходниках enodia, так
что действует дефолтное поведение YAML (строчными буквами, без разбиения
слов). Все остальные поля и так состоят из одного слова (`type`, `key`,
`regex`, `line`), так что это касается только составного — пиши
`cleanregex`, целиком строчными. Проверено напрямую по парсеру (ключ
`cleanRegex` отклоняется как неизвестное поле), а не предположено ни по
комментариям в коде, ни по внутренним дизайн-документам проекта — там
обоих местах в прозе используется `cleanRegex`.
:::
...
будет использован не тот файл (или вообще никакой). Запусти
`enodia config path`, чтобы увидеть, какой файл реально будет
использован.

**`settings.yaml`** — та же идея, с двумя отличиями: дополнительно
проверяется голое имя `settings.` (не только `enodia.settings.`), и
отсутствие файла вообще — **не** ошибка: каждое поле просто откатывается
к встроенному значению по умолчанию, поскольку этот файл полностью
опционален:
...
7. `./.settings.yaml`
8. `./.settings.yml`
9. `$XDG_CONFIG_HOME/enodia/settings.yaml` (`~/.config/enodia/settings.yaml`, если `$XDG_CONFIG_HOME` не задан)
