...
defaults:                            # опционально
  ...
  retries: 2
  backoff: 500ms
cve:                                 # опционально, см. «Сопоставление с CVE»
  bdu:
    path: vulxml.zip
  nvd:
    path: nvd/
...
и/или JSON-фиды NVD (`cve.nvd.path`), которые вы скачали сами, — enodia
их никогда не загружает. Относительные пути резолвятся относительно
...
`options` — произвольная карта для конкретного продукта; большинство
проб её вообще игнорируют. [`p4d`/`p4p`](/ru/configuration/products/p4d/)
— первые, кто реально читает из неё значение: `options.binary`
переопределяет путь к CLI `p4`, к которому они обращаются.

Раздел **«Настройка продуктов»** в боковом меню (или таблица
[Поддерживаемые продукты](/ru/products/)) содержит точный эндпоинт,
требования к аутентификации и записываемые поля для каждой из 90
...
## Интеграция с HashiCorp Vault Agent

Ни встроенную карту `credentials:` в `enodia.yaml`, ни отдельный
`credentials.yaml` не обязательно писать вручную. Оба — это просто
файлы, которые enodia читает заново при каждом запуске — подтверждено
по исходному коду: `enodia check` перечитывает конфиг и credentials с
нуля при каждом вызове, а `enodia serve --interval` делает то же самое
на каждом цикле обновления (`Config.Build` вызывает `LoadCredentials`
каждый раз, когда выполняется `collectObservations` — ничего не
кешируется на время жизни процесса, поэтому изменения в любом из этих
файлов вступают в силу без перезапуска). Это ровно та форма, под
которую заточен собственный рендеринг шаблонов у
[Vault Agent](https://developer.hashicorp.com/vault/docs/agent-and-proxy/agent).
У enodia нет собственной интеграции с Vault — она и не нужна, оба
механизма ниже уже сочетаются с ним напрямую.

### Vault Agent рендерит переменные окружения

Направьте блок `template` (или `env_template`) Vault Agent на секреты,
которые нужны таргету, и ссылайтесь на них обычным способом — через
[подстановку переменных окружения](#подстановка-переменных-окружения)
выше:
...
Режим `exec` у Vault Agent запускает саму enodia (или обёрточный скрипт,
вызывающий `enodia check`) как свой управляемый дочерний процесс,
вставляя отрендеренные переменные прямо в окружение этого процесса —
секрет вообще не попадает на диск как файл, который enodia должна
прочитать. Блок `exec` у Vault Agent также поддерживает перезапуск
дочернего процесса при изменении отрендеренного секрета, если вы хотите,
чтобы долгоживущий `enodia serve` подхватил обновлённый токен сразу, а
не дожидался, пока он просто останется валидным до следующего тика
`--interval` — точную настройку для этого смотрите в собственной
документации Vault Agent, это целиком на его стороне.

### Vault Agent рендерит `credentials.yaml` напрямую

Укажите `credentials_file:` на путь, по которому блок `template` Vault
Agent записывает файл, и отрендерите ровно ту форму, которую ожидает
...
```hcl title="Блок шаблона Vault Agent — иллюстративно; точный синтаксис смотрите в документации Vault Agent"
template {
...
Этот путь вообще не требует настройки `exec`/перезапуска: `enodia check`
перечитывает `credentials_file` с нуля при каждом вызове, а `enodia
serve` — на каждом цикле обновления, независимо от того, как именно файл
изменился на диске. И cron-задача с `enodia check`, и долгоживущий
`enodia serve` просто подхватывают то, что Vault Agent записал
последним, по собственному расписанию — ничего специфичного для enodia
настраивать не нужно.

### В любом случае — по правилам обработки credentials у enodia

Оба варианта всё равно попадают под всё, что уже описано в
[Безопасности](/ru/security/) — credentials никогда не попадают в
инвентарь, экспортированные отчёты или логи, а проверка TLS остаётся
включённой, если вы не откажетесь от неё для конкретного таргета.
Именно настройки `perms` и выбор директории назначения у самого Vault
Agent защищают отрендеренный файл от доступа со стороны — у самой
enodia нет никакого мнения о том, где лежит `credentials_file`, кроме
того, что относительный путь резолвится относительно конфига, который
на него ссылается.
