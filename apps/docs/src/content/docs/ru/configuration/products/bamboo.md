...
Пока не подключён — и, в отличие от других продуктов из этого списка
без резолвера, это не потому, что у endoflife.date нет подходящего
календаря: [endoflife.date/bamboo](https://endoflife.date/bamboo)
существует и содержит реальные данные по циклам, подтверждено через
`endoflife.date/api/bamboo.json`. В `registry.go` для этого продукта
просто указано `resolver: ""`, без комментария «confirmed 404», который
есть у соседних проб Atlassian — похоже на пробел в самой enodia, а не
на намеренное «календаря не существует». enodia всё равно отслеживает
`version` и `buildNumber` независимо от этого; неопределённой остаётся
только ось lifecycle, пока это не подключат в апстриме.
