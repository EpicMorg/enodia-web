...
Сверяется с NVD и БДУ ФСТЭК, если настроен [блок `cve:`](/ru/cve/). Сопоставление по баннеру: `OpenSSH_…` — как OpenSSH, `dropbear_…` — как Dropbear; любая другая реализация SSH остаётся без сверки, а не получает CVE от OpenSSH.
