...
На Windows также можно поставить через
[Chocolatey](https://community.chocolatey.org/packages/enodia), если
хотите, чтобы обновления отслеживал пакетный менеджер (пакет для winget
в пути, пока не опубликован):
...
**Начиная с 1.1.0 этот образ собирает и публикует соседний репозиторий**,
[EpicMorg/docker](https://github.com/EpicMorg/docker/tree/master/linux/ecosystem/apps/enodia),
по собственному расписанию — уже не пайплайн релизов этого проекта,
хотя опубликованный адрес и теги остались теми же. Публикуется также в
`docker.io/epicmorg/enodia` и Quay, те же теги — `latest`, голый тег
мажорной версии (`2`) и точная версия без суффикса сборки (например,
`2.0.0` — подтверждено вживую во всех трёх реестрах; у более раннего пайплайна теги выглядели
как `1.0.0-1`, они всё ещё доступны для pull, просто новые релизы с
этого момента тегируются иначе). Два реальных изменения, о которых
стоит знать: теперь образ собирается **только под `linux/amd64`**
(arm64 убрали при переносе публикации), и работает от **root**, а не
от отдельного пользователя, на собственной домашней базе проекта
`debian:trixie-light` вместо `scratch`.
